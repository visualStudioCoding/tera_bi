        ...
        for (int idx = 0; idx < classification.length(); idx++) {
            ...
            for (Object data : columns) {
                ...
                if("코스피지수".equals(index)){
                    commonService.insertContents(dataMap, PROGRAM_ID + ".insertKospi");
                } else {
                    commonService.insertContents(dataMap, PROGRAM_ID + ".insertKosdaq");
                }
            }
        }
        ...
        for (int idx = 0; idx < categoryGroup.length(); idx++) {
            ...
            for (int i = 0; i < category.length(); i++) {
                ...
                if("출생아수".equals(name) || "사망자수".equals(name)) {
                    ...
                    for (Object data : columns) {
                        ...
                        dataList.add(dataMap);
                        if("출생아수".equals(name)){
                            commonService.insertContents(dataMap, PROGRAM_ID + ".insertBirth");
                        } else {
                            commonService.insertContents(dataMap, PROGRAM_ID + ".insertDeath");
                        }
                    }
                }
            }
            if("기대수명".equals(innerCategoryGroup.get("이름"))){
                ...
                for (Object data : jsonArray) {
                    ...
                    dataList.add(dataMap);
                    commonService.insertContents(dataMap, PROGRAM_ID + ".insertLifeExpectancy");
                }
            }
        }
        result.put("data", dataList);
